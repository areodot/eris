...
class Client extends EventEmitter {
   ...
   * @arg {Buffer} options.file[].file A buffer containing file data
   * @arg {String} options.file[].name What to name the file
   * @arg {Object} [options.withComponents] Whether to respect the components field of the request. When enabled, allows application-owned webhooks to use all components and non-owned webhooks to use non-interactive components.
   * @returns {Promise<Message>}
   */
  editWebhookMessage(webhookID, token, messageID, options) {
    ...
    return this.requestHandler.request("PATCH", Endpoints.WEBHOOK_MESSAGE(webhookID, token, messageID) + (qs ? "?" + qs : ""), false, file ? { payload_json: options } : options, file).then((response) => new Message(response, this));
  }
